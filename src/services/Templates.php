...
class Templates extends Component
{
    ...
    {
        // Token-only callers do not need a Craft application or a Twig environment.
...
REGEX;
     ...
     * Craft's shorthand uses `raw`; callers must escape output for its final context.
     */
    ...
    public function renderObjectTemplate(string $template, mixed $object, array $variables = []): string
    {
        ...
        try {
            return $this->_renderObjectTemplate($this->getTwig(), $template, $object, $variables, false);
        } catch (Throwable $e) {
    ...
    public function renderString(string $template, array $variables = []): string
    {
        ...
        try {
            return $this->getTwig()->createTemplate($template)->render($variables);
        } catch (Throwable $e) {
            $this->_logError($template, $e);
            return '';
        }
